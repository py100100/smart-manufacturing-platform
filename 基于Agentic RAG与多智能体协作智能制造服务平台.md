...
| 智能编排 | 自研 `AgentOrchestrator`，负责自动路由、指定智能体调用、多智能体协同和节点反馈聚合 |
| RAG 检索 | `KnowledgeService`，优先 LangChain + Chroma，失败时回退 TF-IDF |
| 模型调用 | `DeepSeekClient`，通过环境变量配置；未配置时使用本地专家模板兜底 |
| 业务答案增强 | `BusinessAnswerService`，负责生成结构化专家答案并通过质量门避免空洞摘要 |
| 业务闭环 | `BusinessClosureService`，将智能体结果聚合为预警、工单、报告和行动项 |
| 前端展示 | `AgentResult.vue` 统一展示分析结论、分析路径图、证据、执行链路和业务闭环 |
...
3. Agentic RAG 引擎执行检索、知识召回与上下文构建
4. 专业智能体完成领域分析、推理与建议生成
5. 业务答案质量门检查 `summary` 是否过短或空洞，必要时由 BusinessAnswerService 生成答案综合结果
6. 系统进行规则校验、标准溯源、风险提示与结果打分
7. 输出报告、图表、建议方案或自动触发业务工单
8. 全流程记录审计日志，沉淀至知识库与历史案例库
...
flowchart LR
    ...
    F --> Q{summary 是否合格}
    Q -->|否| QA[答案综合<br/>BusinessAnswerService]
    Q -->|是| G[规则校验 风险识别 标准溯源]
    QA --> G
    G --> H{是否需要处置闭环}
...
- 结果输出前会经过业务答案质量门，避免出现“已接收请求”“请提供完整数据”等空洞摘要
- 当摘要不合格时，BusinessAnswerService 会生成结构化专家答案，并在节点反馈中追加“答案综合”节点
